...
Official TypeScript/JavaScript SDK for Refract - The OpenTelemetry-native AI Observability Platform.
...
const Refract = initRefract({
  ...
  // For Refract Cloud, an API key is required
...
Use `Refract.traceLLM` to automatically instrument an LLM call. It captures the prompt, response, model, token usage, latency, and calculates the cost for you.
...
// Wrap your LLM calls with Refract.traceLLM()
...
console.log('✅ Trace sent to Refract!', response.choices[0].message);
```
...
Modern AI applications are complex pipelines. Use `Refract.trace()` to create a **parent span** for an entire workflow, like a RAG pipeline. Any `traceLLM` calls inside it will automatically become **child spans**.
...
const answer = await Refract.trace('rag_request', async (parentSpan) => {
  ...
  const documents = await Refract.trace('retrieval', async () => {
...
const openai = new OpenAI();
const Refract = initRefract({ service_name: 'nextjs-app' });

export async function POST(req: Request) {
  const { message } = await req.json();

  // The 'trace' method creates a parent span for the entire request
  const response = await Refract.trace('POST /api/chat', async () => {
    return await Refract.traceLLM(
      () =>
        openai.chat.completions.create({
          model: 'gpt-4',
...
1.  **Initialize once**: Create a single `Refract` instance and reuse it throughout your application.
2.  **Use Parent Spans**: Wrap your business logic (e.g., API request handlers, complex functions) in a `Refract.trace()` block to create parent spans.
...
4.  **Flush on Exit**: Call `Refract.shutdown()` on `SIGTERM`/`SIGINT` to prevent losing traces from a terminating process.
...
- **Missing traces on exit?** Ensure you are calling `await Refract.shutdown()` before the process terminates.
