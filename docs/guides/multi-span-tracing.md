...
While tracing individual LLM calls is useful, modern AI applications are complex systems with multiple steps. To truly understand and debug these systems, you need to trace the entire workflow, not just the individual components. Refract achieves this through **hierarchical, multi-span tracing**.
...
This creates a clear, hierarchical view in the Refract dashboard, allowing you to see both the total time and cost of the entire operation, as well as the performance of each individual step.
...
You can create hierarchical traces using the `Refract.trace()` method. Any `trace` or `traceLLM` calls made inside its callback will automatically become child spans of the outer trace.
...
const Refract = initRefract({ service_name: 'my-rag-app' });
...
async function answerQuestionWithRAG(query: string) {
  ...
  const answer = await Refract.trace('rag_request', async (parentSpan) => {
    ...
    const documents = await Refract.trace('retrieval', async (retrievalSpan) => {
    ...
    // nested under the 'rag_request' span in the Refract UI.
...
4.  **Clear Visualization**: The Refract dashboard will visualize these traces as a waterfall or flame graph, making the flow of your application intuitive to understand.
