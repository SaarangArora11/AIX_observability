...
import { loadSettings, saveSettings, DEFAULT_SETTINGS, type ApiSettings } from "@/lib/refract/settings";
...
export function SettingsDrawer() {
  ...
  const save = () => { saveSettings(s); setOpen(false); };
    ...
    <Drawer open={open} onOpenChange={setOpen} direction="right">
      ...
      <DrawerContent className="ml-auto h-full w-full max-w-md bg-card/80 backdrop-blur-2xl border-l border-white/10">
        <DrawerHeader>
          ...
          <DrawerDescription>
            Point the dashboard at your Refract services. Preview can't reach <code className="font-mono-tight text-xs">localhost</code> directly — run the app locally or use a tunnel URL.
          </DrawerDescription>
        </DrawerHeader>
          ...
          {fields.map((f) => (
            <div key={f.key} className="space-y-1.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => test(f.key)} className="text-xs text-primary hover:underline">test</button>
              </div>
            </div>
          ))}
        </div>
        <DrawerFooter>
          ...
          <Button variant="ghost" onClick={() => setS(DEFAULT_SETTINGS)}>Reset to defaults</Button>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
  );
